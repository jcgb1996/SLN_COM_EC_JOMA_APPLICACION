...
namespace SLN_COM_EC_JOMA_APPLICACION.Middleware
{
    public class SessionManagementMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            {
                if (!path.Contains($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_LOGIN.ToLower()}") &&
                    !path.Contains($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_CERRARSESION.ToLower()}"))
                {
                    context.Session.Clear(); // Limpiar la sesión por seguridad
                    context.Response.Redirect($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_LOGIN}");
                    return;
                }
            }
            else
            {
                ...
                if (path.Contains($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_CERRARSESION.ToLower()}"))
                {
                ...
                if (path.Contains($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_LOGIN.ToLower()}"))
                {
                    context.Response.Redirect($"/{WebSiteConstans.JOMA_WEBSITE_AREA_INICIO_CONTROLLER_DASHBOARD}");
                    return;
                }
            }
